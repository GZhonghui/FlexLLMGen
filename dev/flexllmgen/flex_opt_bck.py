...
"""
Usage:
python3 flex_opt.py --model facebook/opt-13b --gpu-batch-size 16 --num-gpu-batches 1 --prompt-len 512 --gen-len 2 --percent 100 0 100 0 100 0 --debug-mode breakdown
"""

import warnings
# Suppress FutureWarning from torch.distributed
...
class InputEmbed:
    ...
    # 输入输出也有forward函数，保持和TransformerLayer一样的接口，方便编排
...
class SelfAttention:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        # mha_gen的输入和mha略有不同，mha_gen的输入是当前token的hidden和整个cache，而mha的输入是当前所有已生成token的hidden和整个cache，所以需要区分开来，prefill阶段用mha，decoding阶段用mha_gen
        else:  # decoding
...
class MLP:
    ...
    # 其实attention_mask并没有用到，不用传的
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        donate = [False] * 7
        h, donate[0] = hidden.val, True
        # donate表示是否可以原地更新这个变量，能原地更新就尽量原地更新，减少内存占用和数据复制（不太懂）
        ...
        # 实际计算
        # compute是一个Device对象，里面有各种计算函数，这个mlp函数就是实现了MLP的前向计算，输入是hidden和权重，输出是新的hidden
        # print(f"[DEBUG] MLP forward is called, compute device: {self.compute}", flush=True)
        # print(f"[DEBUG] About to call self.compute.mlp", flush=True)
        self.compute.mlp1() # DEBUG
...
class TransformerLayer:
    ...
    # Transformer layer的前向计算，包含attention和mlp
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k):
        # attn和ffn没有分开，那weight就是一起加载
        if k == self.policy.num_gpu_batches - 1:
            read_buf1, read_buf2 = weight_read_buf.pop()
        else:
            read_buf1, read_buf2 = weight_read_buf.val

        # 注意这里hidden是in-place更新的，cache_read_buf和weight_read_buf是只读的，cache_write_buf是只写的
        self.attention.forward(hidden, cache_read_buf, read_buf1, attention_mask,
                               cache_write_buf, i, k)
        # ffn不需要cahce，所以cache_read_buf和cache_write_buf都传None
...
class OptLM:
    def __init__(self,
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer: # 是否分开attention和mlp成两层
        ...
        self.layers = layers
        self.num_layers = len(layers) # 实际的层数可能是config.num_hidden_layers的2倍（如果sep_layer=True）加上输入输出嵌入层
        ...
        # cache[j][k]
        # 二维数组，每一个元素是一个ValueHolder，存储第j层第k个gpu batch的cache
    ...
    def set_task(self, task):
        ...
        for l in self.layers:
            l.set_task(task) # 为每一层设置task，可能会用到task中的一些信息（比如prompt_len, gen_len等）
    ...
    # 计算第i个token经过第j层第k个gpu batch的结果
    def compute_layer(self, i, j, k):
        # print(f"[DEBUG] Computing layer {j} for token {i} on gpu batch {k}")  # DEBUG
        ...
        # 这个forward需要hidded，cache，weight
        # 这个hidden的形状是不确定的吧
        # KV Cache和token位置无关，和layer和gpu batch有关，所以是cache_read_buf[j][k]
        # weight是和层有关的，所以是weight_read_buf[j]
        # 要写出的cache，是写出全量的cache吗？？
        # 这个forward是虚函数？
    ...
    def generate(self,
        ...
        task = Task(
            inputs=inputs,
            prompt_len=len(inputs[0]), # 所有的输入长度都必须相同
        ...
        num_gpu_batches = self.num_gpu_batches # 这个不是batch size，而是block size
        gpu_batch_size = self.policy.gpu_batch_size # 这个才是batch size
        ...
        # Output token ids
        # 输出的是id
        # 为每一个input准备一个输出buffer，长度是prompt_len + gen_len，前面prompt_len个位置存输入的id，后面gen_len个位置存生成的id
        self.output_ids = np.full((len(task.inputs), prompt_len + gen_len),
            self.config.pad_token_id, dtype=np.int32)
        self.stopped = np.zeros((len(task.inputs), 1), dtype=bool)
        self.output_ids[:, :prompt_len] = np.asarray(task.inputs)
        # len(task.inputs)就是requests的数量
        ...
        if debug_mode is None:
            if not overlap:
                ...
                if num_gpu_batches == 1:
                    ...
                    # 开启overlap，并且有多个gpu batch，这时候是z字形的schedule
                    # 最核心的计算方法
    ...
    def generation_loop_debug_normal(self):
        ...
        timers("compute_attention_prefill").reset()
        timers("compute_attention_decoding").reset()
        timers("compute_mlp_prefill").reset()
        timers("compute_mlp_decoding").reset()

        timers("prefill_mlp_debug").reset()
        ...
        for i in range(self.execute_gen_len):
            if i == 0:
                ...
                compute_attention_timer = timers("compute_attention_prefill")
                compute_mlp_timer = timers("compute_mlp_prefill")
            else:
                ...
                compute_attention_timer = timers("compute_attention_decoding")
                compute_mlp_timer = timers("compute_mlp_decoding")
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    ...
                    # Identify layer type and time accordingly
                    layer = self.layers[j]
                    compute_layer_timer.start(self.sync)
                    if isinstance(layer, SelfAttention):
                        compute_attention_timer.start(self.sync)
                        self.compute_layer(i, j, k)
                        compute_attention_timer.stop(self.sync)
                    elif isinstance(layer, MLP):
                        compute_mlp_timer.start(self.sync)
                        self.compute_layer(i, j, k)
                        compute_mlp_timer.stop(self.sync)
                    else:
                        self.compute_layer(i, j, k)
        ...
        for stage in ["prefill", "decoding"]:
            for func in ["load_cache", "store_cache", "compute_layer"]:
                ...
                total = np.sum(costs)
                avg = np.mean(costs)
                print(f"{name:22s}: total={total:.6f}s, avg={avg:.6f}s (n={len(costs)})")
            # Print breakdown of attention and mlp
            for func in ["compute_attention", "compute_mlp"]:
                name = func + "_" + stage
                costs = timers(name).costs
                if costs:
                    total = np.sum(costs)
                    avg = np.mean(costs)
                    print(f"  {name:20s}: total={total:.6f}s, avg={avg:.6f}s (n={len(costs)})")
        
        # DEBUG
        prefill_mlp_debug_t = timers("prefill_mlp_debug")
        print(f"prefill_mlp_debug: total={np.sum(prefill_mlp_debug_t.costs):.6f}s, avg={np.mean(prefill_mlp_debug_t.costs):.6f}s (n={len(prefill_mlp_debug_t.costs)})")
    ...
    def generation_loop_overlap_multi_batch(self):
        ...
        # Generate
        # 循环顺序是i, j, k
        # i是生成的token的位置，j是层数，k是gpu batch数
        for i in range(self.execute_gen_len):
            timers("generate").start()
            for k in range(self.num_gpu_batches):
                self.update_attention_mask(i, k)
            for j in range(self.num_layers): # 这个layer不一定是Attn
                for k in range(self.num_gpu_batches):
                    # 加载下一层权重
                    self.load_weight(i, j+1, k)
                    # 加载下一个batch的cache
                    self.load_cache(i, j, k+1)
                    self.store_hidden(i, j, k-1) # hidden无视
                    self.load_hidden(i, j, k+1) # hidden无视
                    self.compute_layer(i, j, k) # 计算（Attn或者FFN）
                    self.store_cache(i, j, k-1) # 卸载上一个batch的cache（为什么在这里卸载？不是更早卸载？）
                    self.sync() # 这里同步，保证下一次计算之前，数据已经准备好了
            timers("generate").stop()
